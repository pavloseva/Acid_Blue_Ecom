      ...
      <div className="pt-28 pb-20">
        <div className="max-w-7xl mx-auto px-6 lg:px-8">
          ...
          <div className="text-center mb-12">
            ...
            <h1 className="font-serif text-4xl md:text-5xl lg:text-6xl text-foreground mb-4 text-balance font-semibold">
              Toda la tienda
            </h1>
            ...
            <div className="lg:hidden fixed inset-0 z-50 bg-background">
              <div className="p-6">
                <div className="flex items-center justify-between mb-8">
                  <h2 className="font-serif text-2xl text-foreground font-semibold">Filtros</h2>
                  <button
                    type="button"
                ...
                <div className="space-y-3">
                  {filters.map((filter) => (
                    <button
                      ...
                      className={`w-full px-6 py-4 rounded-2xl text-left boty-transition border ${
                        selectedCategory === filter.value
                          ? "bg-primary text-primary-foreground border-primary"
...
function ProductCard({
  ...
  return (
    ...
    >
      <div className="bg-card rounded-3xl overflow-hidden boty-shadow boty-transition group-hover:scale-[1.02] group-hover:acid-glow border border-border">
        {/* Image */}
        <div className="relative aspect-square bg-muted overflow-hidden">
          ...
          <button
            type="button"
            className="absolute bottom-4 right-4 w-12 h-12 rounded-full bg-primary text-primary-foreground flex items-center justify-center opacity-0 translate-y-2 group-hover:opacity-100 group-hover:translate-y-0 boty-transition"
            onClick={(e) => {
              e.preventDefault()
              e.stopPropagation()
              addItem({
        ...
        {/* Info */}
        <div className="p-6">
          <h3 className="font-serif text-xl text-foreground mb-1 font-semibold">{product.name}</h3>
          <p className="text-sm text-muted-foreground mb-4">{product.description}</p>
          <div className="flex items-center gap-2">
            <span className="text-lg font-medium text-foreground">{formatARS(product.price)}</span>
